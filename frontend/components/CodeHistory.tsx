    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 sm:px-3 py-2 bg-slate-700 hover:bg-slate-600 text-white text-xs sm:text-sm rounded transition-colors"
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 sm:px-3 py-2 bg-red-900/30 hover:bg-red-900/50 text-red-300 text-xs sm:text-sm rounded transition-colors"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
          ...
          <div className="bg-slate-900/50 rounded-lg p-3 sm:p-4">
            <div className="text-xs sm:text-sm text-slate-400 mb-1">Total Reviews</div>
            <div className="text-2xl sm:text-3xl font-bold text-white">{history.length}</div>
          </div>

          {/* Average Score */}
          <div className="bg-slate-900/50 rounded-lg p-3 sm:p-4">
            <div className="text-xs sm:text-sm text-slate-400 mb-1">Average Score</div>
            <div className="flex items-baseline gap-2">
              <div className="text-2xl sm:text-3xl font-bold text-white">{avgScore}</div>
              <div className="text-xs sm:text-sm text-slate-400">/100</div>
            </div>
          </div>

          {/* Trend */}
          <div className="bg-slate-900/50 rounded-lg p-3 sm:p-4">
            <div className="text-xs sm:text-sm text-slate-400 mb-1">Trend</div>
      ...
      <div className="bg-slate-800/50 border border-slate-700 rounded-lg overflow-hidden">
        <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-slate-700">
          <h4 className="text-base sm:text-lg font-semibold text-white">Recent Reviews</h4>
        </div>
        ...
        <div className="divide-y divide-slate-700 max-h-96 overflow-y-auto">
          {history.length === 0 ? (
            <div className="px-4 sm:px-6 py-12 text-center text-slate-400">
              ...
              <p className="text-sm sm:text-base">No reviews yet. Start reviewing code to build your history!</p>
            </div>
          ) : (
            history.map((item) => (
              <div
                key={item.id}
                className="px-4 sm:px-6 py-3 sm:py-4 hover:bg-slate-700/30 cursor-pointer transition-colors"
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
                  <div className="flex-1">
                    ...
                    <pre className="text-xs sm:text-sm text-slate-300 font-mono bg-slate-900/50 p-2 rounded overflow-x-auto">
                      {item.codeSnippet}
                    </pre>
                  </div>
                  <div className="sm:ml-4 self-end sm:self-start">
