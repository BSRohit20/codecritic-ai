...
async def send_verification_email(email: str, token: str):
    """Send email verification link to user via SendGrid HTTP API"""
    try:
        ...
        if not SENDGRID_API_KEY:
            print(f"⚠️  SENDGRID_API_KEY not configured!")
            print(f"📧 Email verification link: {verification_link}")
            return {"success": False, "error": "SENDGRID_API_KEY not configured", "dev_mode": True, "link": verification_link}
        ...
        payload = {
            ...
            "from": {"email": FROM_EMAIL, "name": "CodeCritic AI"},
            "content": [
                {
                    "type": "text/html",
                    "value": f"""
        ...
        print(f"📧 Sending verification email to {email} via SendGrid HTTP API...")
...
async def send_welcome_email(email: str, name: str):
    """Send welcome email after verification via SendGrid HTTP API"""
    try:
        if not SENDGRID_API_KEY:
            print(f"⚠️  SENDGRID_API_KEY not configured! Welcome email will not be sent.")
        ...
        print(f"📧 Sending welcome email to {email} via SendGrid HTTP API...")
